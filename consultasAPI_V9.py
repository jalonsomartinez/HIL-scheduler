...
class IstentoreAPI:
    ...
    def day_ahead_schedule_between_periods(self, delivery_period_gte: str = None, delivery_period_lte: str = None):
        ...
        for period in data.get("delivery_periods", []):
            ...
            lib_day_ahead_setpoints_kW[delivery_period_iso_utc] = (activation.get("lib_to_vpp_kw", 0.0) - activation.get("vpp_to_lib_kw", 0.0))

            vrfb_day_ahead_setpoints_kW[delivery_period_iso_utc] = (activation.get("vrfb_to_vpp_kw", 0.0)- activation.get("vpp_to_vrfb_kw", 0.0))
    ...
    def mfrr_next_activation(self):
        ...
        now_utc = datetime.now(ZoneInfo("Europe/Madrid")).astimezone(ZoneInfo("UTC")).isoformat()
        data_list = self._get_market_products(market_id=3, delivery_period_gte=now_utc)
        ...
        # 1. Parsear string UTC (naive) → datetime
        delivery_dt_utc = datetime.strptime(delivery_period_api, "%Y-%m-%d %H:%M:%S").replace(tzinfo=timezone.utc)
        ...
        lib_mfrr_next_activation_kW[delivery_period_iso_utc] = activation.get("total_upward_kw", 0.0) - activation.get("total_downward_kw", 0.0)
