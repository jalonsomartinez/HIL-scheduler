...
4. Add explicit operator alerts for sustained posting failures, stale schedule windows, command-queue saturation, or persistent Modbus read/connect failures.
...
6. Evaluate queue topology evolution (global FIFO vs per-plant queues with global command barriers) after collecting operator observations from new queue/health UI.
